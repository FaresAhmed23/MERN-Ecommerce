...
const Header = () => {
	...
	return (
		<header className="fixed top-0 left-0 m-auto max_padd_container w-full ring-1 bg-white ring-slate-900/5 z-10">
			<div className="px-4 flexBetween py-3 max-xs:px-2">
				...
				<div className="flexBetween sm:gap-x-2 bold-16">
					{!menuOpened ? (
						<MdMenu
							className="md:hidden cursor-pointer hover:text-secondary mr-2 p-1 ring-1 ring-slate-900/5 h-8 w-8 rounded-full"
							onClick={toggleMenu}
						/>
					) : (
						<MdClose
					...
					<div className="flexBetween sm:gap-x-6">
						...
						{localStorage.getItem("auth-token") ? (
							...
							<NavLink
								to={"login"}
								className="btn_secondary_rounded flexCenter gap-x-2 medium-2"
							>
